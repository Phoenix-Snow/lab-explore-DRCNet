...
if __name__ == "__main__":
    ...
    if task == 'train':
        ...
        # dataset: test
        if 'now' in param.CONFIG['run']['pretrained_model_path']:
            print('[INFO] Preprocess dataset (now)')
            ...
            train_dataset = build_test_dataset4now(param.RESOURCE['station_path_dict']['train'],
                                                   param.TIME_SERIES_LENGTH)
            target_mode = 'now'
        elif 'future' in param.CONFIG['run']['pretrained_model_path']:
            print('[INFO] Preprocess dataset (future)')
        ...
        # Model EXP (Register: Model, Loss Function, Optimizer, Trainer/Evaluator/Predictor Function)
        exp = Procedure(param.TIME_SERIES_LENGTH, 'cuda:0', antiNormalizer, Test_Reader, Train_Reader, Spatial_Reader,
                        param.CONFIG['run']['results_save_path'], param.CONFIG['run']['pretrained_model_path'],
                        target_mode, True, target_mode)
