...
from Integration.merge_func.readme_construction import create_dataset_readme
...
from Tools.Tools import normal_city, normal_province, extract_coordinates, extract_coordinates_plus, \
    jsonDumps
...
class MergeByYaml:
    def __init__(self, yaml_path: str):
        ...
        # Tools
        self.__geojson = China_GeoJson()

        # queue
        self.__cleaner_queue = queue.Queue(maxsize=2)
        self.__saver_queue_split = queue.Queue(maxsize=2)  # temporal_resource
        self.__saver_queue_cleaned = queue.Queue(maxsize=2)  # temporalSpacial_series_cleaned.csv
        self.__saver_queue_normal = queue.Queue(maxsize=2)  # temporalSpacial_series_normal.csv
        # signal
        self.___all_data_scanned___ = False
        self.___all_files_merged___ = [False, False, False]
        self.__valid_station_ids = []
        # thread stop event
        self.__stop_event = threading.Event()

        # Guardian
        self.__guardian = ProcessGuardian(os.getpid())

    def load_config(self, state, name: Union[int, str, None] = None):
        print(">>>>>>>>>>>>>> ⚙️ load config >>>>>>>>>>>>>>>>")
        while True:
            ...
            if state == "work":
                ...
                if name is None:  # 使用merge配置时，name使用默认的None
                    if "dataset_name" in config_yaml.keys() and \
                        ...
                        if os.path.exists(os.path.join("datasets", config_yaml["dataset_name"])):
                            ...
                            print(f">> 确认选择: {options[selected_index]}")
                ...
                print(">>>>>>>>>>>>>> ✅ success & next step: -> merge dataset >>>>>>>>>>>>>>>>\n")
                return self.work()
            elif state == "not_work":
                # datasets
                if name is None:
                    raise Exception("[❌ your quotation error] if you don't need reconstruct any dataset, "
                                    "you can use 'not_merge', but the passing parameter 'name' shouldn't be None."
                                    "It must be the dataset name (path base location is '~/datasets/')")
                self.__name = name
                ...
                self.__normalized_data_save_path = os.path.join(dataset, "temporalSpacial_series_normal.csv")
                self.__readme_path = os.path.join(dataset, "README.md")
                print(">>>>>>>>>>>>>> ✅ success & next step: load dataset >>>>>>>>>>>>>>>>\n")
    ...
    def work(self, time_series_len=None, temporal_series_path=None, dem_path=None):
        ...
        try:
            ...
            self.__stop_event.set()
            heartbeat_thread.join()
            self.__guardian.stop()

        if all(self.___all_files_merged___) and self.__station_ID_list is not None:
            print(">>>>>>>>>>>>>> 🎉 merge end >>>>>>>>>>>>>>>>\n")
        else:
        ...
        # readme construction
        readme = create_dataset_readme(self.__name, self.__station_3d)
        readme.generate(self.__readme_path)
    ...
    def provider_thread(self, ):
        print("👮‍♂️ [provider] 启动...")
        # 向守护进程汇报TID
        self.__guardian.record_TID(threading.get_ident(), '👮‍♂️ provider')
            ...
            for f_name in files:
                ...
                if station_id not in self.__station_ID_list and station_id not in drop_station:
                    if len(self.__columns['spatial_column']) == 2:
                        ...
                        loc = self.__geojson.IDENTIFY_PROVINCE(coordinates[0], coordinates[1])
                        if self.__domain_type == "city" or "市" in self.__domain_type:
                            loc = normal_city(loc)
                        else:
                ...
                self.__cleaner_queue.put({str(station_id): {'input': input_data, 'output': output_data}})
            self.___all_data_scanned___ = True
        else:
            # ===============================
            # === 🔍 筛选, 分站点保存临时文件 ===
            # ===============================
            ...
            for input_chunk, output_chunk in zip(input_reader, output_reader):
                    ...
                    if station_id not in self.__station_ID_list and station_id not in drop_station:
                        ...
                        try:
                            ...
                            if coordinates is not None:
                                ...
                                # 调用你的地理围栏逻辑
                                loc = self.__geojson.IDENTIFY_PROVINCE(lon, lat, )
                ...
                if chunk_station_buffer:
                    for sid in chunk_station_buffer.keys():
                        ...
                        self.__saver_queue_split.put(task_package)
            self.___all_data_scanned___ = True
        ...
        self.__cleaner_queue.put({'STOP': None})  # 显性终止符, 独立,单次
    ...
    def worker_thread(self):
        print("👷 [worker] 启动...")
        self.__guardian.record_TID(threading.get_ident(), '👷 worker')
        ...
        while True:
            try:
                chunks = self.__cleaner_queue.get()
                sids = chunks.keys()
                if 'STOP' in sids and chunks['STOP'] is None:
                    if len(sids) != 1:
                        self.__stop_event.set()
                        raise Exception(
                            '[❌ code error] worker_thread called, provider_thread error, "self.__cleaner_queue.put" stop signal')
                    if not self.___all_data_scanned___:
                        self.__stop_event.set()
                        raise Exception(
                            '[❌ code error] worker_thread called, provider_thread error, "self.___all_data_scanned___" not changed')
                    print("👷 [worker] 文件读取结束。")
                    self.__saver_queue_split.put({
                        'data': None,
                        'mode': "STOP"
                    })  # 显性终止符
                    self.__saver_queue_cleaned.put({
                        'data': None,
                        'mode': "STOP"
                    })
                    self.__saver_queue_normal.put({
                        'data': None,
                        'mode': "STOP"
                    })
                    break
                ...
                #     if self.___all_data_scanned___:
                #         print("👷 [cleaner] 文件读取结束。")
                #         self.__cleaner_queue.put(None)  # 发送结束信号
                #         break
                #     continue
                ...
                for sid in sids:
                    ...
                    self.__saver_queue_cleaned.put(task_package)
                    station_buffer = (station_buffer - cur_min) / (cur_max - cur_min)
                    task_package = {
                        'data': station_buffer,
                        'mode': 'a',
                    }
                    self.__saver_queue_normal.put(task_package)
                    # 清除一次task的计数
                    self.__cleaner_queue.task_done()
            except Exception as e:
                self.__stop_event.set()
    ...
    def consumer_thread_split(self):
        ...
        self.__guardian.record_TID(threading.get_ident(), '✏️ writer 1')
        while True:
            try:
                task_package = self.__saver_queue_split.get()
        ...
        self.___all_files_merged___[0] = True

    def consumer_thread_cleaned(self):
        print("🖍️ [writer 2] 启动...")
        self.__guardian.record_TID(threading.get_ident(), '🖍️ writer 2')
        while True:
            try:
                task_package = self.__saver_queue_cleaned.get()
                mode = task_package['mode']
                if mode == "STOP":
                    break
                station_buffer = task_package["data"]
        ...
        self.___all_files_merged___[1] = True

    def consumer_thread_normal(self):
        print("🖋️ [writer 3] 启动...")
        self.__guardian.record_TID(threading.get_ident(), '🖋️ writer 3')
        while True:
            try:
                task_package = self.__saver_queue_normal.get()
                mode = task_package['mode']
                if mode == "STOP":
                    break
                station_buffer = task_package["data"]
        ...
        self.___all_files_merged___[2] = True

    def _heartbeat_loop(self):
        """后台线程：不断发送心跳"""
        while not self.__stop_event.is_set():  # 需要一个 __stop_event 来控制这个线程
            self.__guardian.record_heartbeat()
