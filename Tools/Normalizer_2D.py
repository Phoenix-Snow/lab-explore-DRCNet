...
class StdMeanAntiNormalize(nn.Module):
    def __init__(self, mean_params, std_params):
        ...
        # Reshape params for broadcasting with 2D data
        self.__mean = self.__mean.reshape(-1, 1, 1)  # (B, C, H, W)
        self.__std = self.__std.reshape(-1, 1, 1)
        self.device = "cpu"

    def location(self, location_index_list: list):
        self.__mean = self.__mean[location_index_list, 0, 0].reshape(-1, 1, 1)
        self.__std = self.__std[location_index_list, 0, 0].reshape(-1, 1, 1)
...
class StdMeanNormalize(nn.Module):
    def __init__(self, mean_params, std_params):
        ...
        self.__std = torch.tensor(std_params, dtype=torch.float32)
        # Reshape params for broadcasting with 2D data
        self.__mean = self.__mean.reshape(-1, 1, 1)
        self.__std = self.__std.reshape(-1, 1, 1)
...
class MinMaxAntiNormalize(nn.Module):
    def __init__(self, min_params, max_params):
        ...
        # Reshape params for broadcasting with 2D data
        self.__max = self.__max.reshape(-1, 1, 1)
        self.__min = self.__min.reshape(-1, 1, 1)
        self.device = 'cpu'

    def location(self, location_index_list: list):
        self.__max = self.__max[location_index_list, 0, 0].reshape(-1, 1, 1)
        self.__min = self.__min[location_index_list, 0, 0].reshape(-1, 1, 1)
